...
def test_suggestions_match_explicit_step_and_ignore_other_context_id(isolated_db):
    ...
    assert found == [{"step_id": step, "title": "Do work", "commits": [{"sha": "abcdef12", "date": "2026-01-02T00:00:00+00:00", "subject": f"feat: finish step {step}", "reason": f"referencia explícita al paso #{step}"}]}]
