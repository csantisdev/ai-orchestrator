...
**Fecha:** 2026-07-13
**Baseline de código auditado:** `e0189a3827f82524942cecaa3929a9c0ddd24487` — todas las citas de archivo:línea de la Parte II están verificadas contra este commit, y el código de `orchestrator/`, `tests/`, `pyproject.toml` y `.github/` no cambió desde entonces (verificado: cero diffs en esos paths entre `e0189a3` y `production` hoy).
...
Codex ejecutó una pasada de verificación de solo lectura (sin escribir código) contra `production@e0189a3`, pidiéndole confirmar cada cita de archivo:línea de la Parte II y las 14 invariantes del Apéndice A. Encontró:
...
- El baseline pre-gate comparable es el `111 passed, 1 failed` (112 tests totales) que RFC-006 mismo documenta. **Verificado: coincide exactamente en cantidad con los `112 passed, 0 failed` de este checkout** (confirmado además que ningún test se agregó entre `e0189a3` y hoy: `tests/test_validate_pricing_catalog.py`, el único candidato a "test nuevo" que se había mencionado en una versión anterior de este párrafo, ya existía en el commit `8929138`, ancestro de `e0189a3` — esa explicación previa era incorrecta y quedó corregida).
...
- **Deriva entre este documento y el código real.** Cada commit cita líneas verificadas el 2026-07-12 contra `e0189a3`. Cada prompt Codex pide releer el archivo antes de editar, no asume el número de línea a ciegas.
