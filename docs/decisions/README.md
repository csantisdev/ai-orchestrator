...
**Análisis vigente:** ANL-001 define el protocolo local y anonimizado para evaluar modelos en generación y validación de tests. ANL-002 define la auditoría cruzada de solo lectura entre Codex y Claude sobre el repositorio completo (todavía no ejecutada). ANL-003 describe la auditoría por cambio en dos rondas que se aplica a cada PR no trivial. `evidence/RFC-006/` solo tiene el README de qué se espera; `evidence/RFC-007/` guarda además los logs de pytest del cierre de Fase 1 y la PoC reproducible — separadas porque RFC-006 es el diseño histórico (I1-I14, patch efímero nunca publicado) y RFC-007 es quien gobierna la reconstrucción real contra código (los 13 commits, I1-I15, el PR #2). Ver `evidence/RFC-007/README.md` para la tabla de trazabilidad.
